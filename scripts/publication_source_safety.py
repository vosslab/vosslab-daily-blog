"""Apply the deterministic publication-source safety policy."""
...
EVIDENCE_COMMENT_RE = re.compile(r"<!-- evidence: ev-[0-9a-f]{16}(?:, ev-[0-9a-f]{16})* -->")
FENCE_START_RE = re.compile(r"(?m)^[ \t]{0,3}(`{3,}|~{3,})[^\n]*\n")
ATTRIBUTE_LIST_RE = re.compile(r"(?<!\\)\{[ \t]*:?[ \t]*(?:[.#][^ \t{}\n]+|[^ \t={}][^ \t={}]*[ \t]*=)[^{}\n]*\}")
ESCAPE_RE = re.compile(r"\\([!\"#$%&'()*+,\-./:;<=>?@[\\\]^_`{|}~])")
REFERENCE_DEFINITION_START_RE = re.compile(r"^[ \t]{0,3}\[([^\]\n]+)\]:[ \t]*(.*)$")
...
RAW_HTML_RE = re.compile(r"</?[a-z][^<>]*>|<!\[cdata\[[\s\S]*?\]\]>|<![^<>]*>|<\?[\s\S]*?\?>", re.IGNORECASE)
...
# This is a behavioral specification, not a set of implementation examples. The producer
...
def policy_vector_bytes() -> bytes:
	...
	return json.dumps(POLICY_VECTOR, ensure_ascii=True, separators=(",", ":"), sort_keys=True).encode("utf-8")


#============================================
def policy_identity() -> dict[str, str]:
	"""Return the portable version and digest carried by every v8 bundle."""
	return {"version": POLICY_VERSION, "sha256": hashlib.sha256(policy_vector_bytes()).hexdigest()}


#============================================
def _inert_source(source: str) -> str:
	"""Mask fenced and inline code without inspecting syntactically inert text."""
	masked = list(source)
	position = 0
	while True:
		match = FENCE_START_RE.search(source, position)
		if match is None:
			break
		marker = match.group(1)
		end = re.compile(r"(?m)^[ \t]{0,3}" + re.escape(marker[0]) + "{" + str(len(marker)) + r",\}[ \t]*$").search(source, match.end())
...
def _url_target(value: str) -> str | None:
	...
	if not value or any(character.isspace() or ord(character) < 32 for character in value):
		return None
	value = urllib.parse.unquote(ESCAPE_RE.sub(r"\1", html.unescape(value)))
	if not value or any(character.isspace() or ord(character) < 32 for character in value):
		return None
	return value
...
def _destination_and_title(value: str) -> str | None:
	...
	return destination if tail[-1] == closer and "\n" not in tail else None
...
#============================================
def validate_post_source(post: object, approved_paths: object) -> tuple[str, ...]:
	"""Return bounded categories for source unsafe to cross the publication boundary."""
	if type(post) is not str or type(approved_paths) not in {list, set, frozenset, tuple}:
		return ("invalid_source",)
	if any(type(path) is not str for path in approved_paths):
		return ("invalid_source",)
	front_matter = FRONT_MATTER_RE.match(post)
	if front_matter is None:
		return ("invalid_source",)
	source = _inert_source(post[front_matter.end():])
	reasons = set()
	comment_spans = []
	for match in re.finditer(r"<!--.*?-->", source, re.DOTALL):
		comment_spans.append(match.span())
		if match.group(0) != "<!-- more -->" and EVIDENCE_COMMENT_RE.fullmatch(match.group(0)) is None:
	...
	if "<!--" in source or "-->" in source:
		reasons.add("unsafe_comment")
	if ATTRIBUTE_LIST_RE.search(source):
		reasons.add("markdown_attribute_list")
	approved = frozenset(approved_paths)
	definitions = {}
	definition_lines = set()
	for line_number, line in enumerate(source.splitlines()):
		match = REFERENCE_DEFINITION_START_RE.fullmatch(line)
		if match is None:
			if re.match(r"^[ \t]{0,3}\[[^\]\n]+\]:", line):
		...
		if not label or target is None or not _target_is_allowed(target, approved):
			reasons.add("unsafe_link")
			continue
	...
	for target in inline_targets:
		if not _target_is_allowed(target, approved):
			reasons.add("unsafe_link")
	...
	if set(definitions) != used_definitions:
		reasons.add("unsafe_link")
	angle_spans = []
	for match in ANGLE_RE.finditer(source):
